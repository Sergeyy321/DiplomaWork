...
export default function CalendarNote() {
  ...
  const [events, setEvents] = useState([
    {
      ...
      isItalic: false,
      align: "left",
      reminder: true, // Включено по умолчанию для теста
      x: -180, 
      y: -60
    }
  ]);
  ...
  const stopTabBlinking = () => {
    if (blinkIntervalRef.current) {
      ...
      document.title = originalTitle; // Возвращаем дефолтное имя
    }
  };

  // 🌟 2. МЕХАНИКА СИНТЕЗА ЗВУКА (Audio Context Synth)
  const playAlertSound = () => {
    try {
      ...
      osc.type = "sine"; // Тип звуковой волны (чистый электронный бип)
      osc.frequency.setValueAtTime(587.33, ctx.currentTime); // Нота Ре (D5) для приятного уведомления
      
      gain.gain.setValueAtTime(0.3, ctx.currentTime); // Громкость (30%)
      gain.gain.exponentialRampToValueAtTime(0.01, ctx.currentTime + 0.4); // Плавное затухание звука
      
      osc.connect(gain);
      gain.connect(ctx.destination);
      
      osc.start();
      osc.stop(ctx.currentTime + 0.4); // Длительность звука — 400мс
    } catch (e) {
      console.warn("Audio Context blocked by browser autoplay policy.");
    }
  };

  // 🌟 3. ФОНОВЫЙ ТАЙМЕР ПРОВЕРКИ НАПОМИНАНИЙ (Каждую минуту)
  useEffect(() => {
    // Храним уже сработавшие ID, чтобы не спамить каждую секунду в течение одной и той же минуты
    const triggeredIds = new Set();

    const checkReminders = () => {
      const now = new Date();
      const currentDateStr = now.toISOString().split("T")[0];
      const currentTimeStr = now.toTimeString().slice(0, 5); // Формат "HH:MM"
      ...
      events.forEach(event => {
        if (event.reminder && event.date === currentDateStr && event.time === currentTimeStr) {
          if (!triggeredIds.has(event.id)) {
            ...
            // Запускаем эффекты привлечения внимания
            playAlertSound();
    ...
    // Запускаем проверку каждые 15 секунд для точности
    const checkInterval = setInterval(checkReminders, 15000);
    return () => clearInterval(checkInterval);
  }, [events]);

  // Останавливаем моргание, как только пользователь кликнет в любом месте приложения
  useEffect(() => {
  ...
  const handleCreateFolder = (e) => {
    ...
    setFolders([...folders, { id: newId, title: newFolderTitle.trim(), icon: newFolderIcon }]);
      ...
      <div style={sidebarStyle}>
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            ...
            <button onClick={() => setIsAddingFolder(!isAddingFolder)} style={addFolderToggleBtn}>➕</button>
          </div>
      ...
      <div style={mainContentStyle}>
        <div style={tabHeaderStyle}>
          <div style={{ display: "flex", gap: "10px" }}>
            <button onClick={() => setActiveTab("calendar")} style={{ ...tabButton, background: activeTab === "calendar" ? "#4f46e5" : "#e5e7eb", color: activeTab === "calendar" ? "white" : "#374151" }}>
              📅 Calendar Dashboard
            </button>
            <button onClick={() => setActiveTab("mindmap")} style={{ ...tabButton, background: activeTab === "mindmap" ? "#4f46e5" : "#e5e7eb", color: activeTab === "mindmap" ? "white" : "#374151" }}>
              🧠 Spatial Coordinates Hub
            </button>
        ...
        {activeTab === "calendar" && (
          <div style={calendarCard} className="premium-calendar-wrapper">
            <Calendar
              onChange={setDate}
              value={date}
              onClickDay={handleDateClick}
              tileContent={({ date: tileDate }) => {
                const dayEvents = filteredEvents.filter(e => e.date === tileDate.toISOString().split("T")[0]);
                return (
                  <div style={tileContentContainer}>
                    {dayEvents.map(event => (
                    ...
                    {dayEvents.length > 0 && <div style={dotIndicatorMatrix(dayEvents[0].color)} />}
                  </div>
                );
              }}
            />
          ...
          <div style={fullscreenContentCard}>
            <div style={previewHeader}>
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                ...
                <button onClick={() => { setIsPreviewOpen(false); editNotification(selectedEvent); }} style={{ ...actionBtn, background: "#e5e7eb", color: "#1f2937" }} >✏️ Edit Document</button>
                <button onClick={() => deleteNotification(selectedEvent.id)} style={{ ...actionBtn, background: "#fee2e2", color: "#dc2626" }} >🗑️ Purge</button>
                <button onClick={() => setIsPreviewOpen(false)} style={{ ...actionBtn, background: "#1f2937", color: "white" }} >✕ Exit Screen</button>
              </div>
            </div>
...
// (Стили остаются абсолютно такими же, как в твоем файле)
...
const addFolderToggleBtn = { background: "none", border: "none", color: "#9ca3af", cursor: "pointer", fontSize: "13px" };
...
const dotIndicatorMatrix = (color) => ({
  width: "6px",
  height: "6px",
  borderRadius: "50%",
  background: color,
